...
if __name__ == '__main__':
    ...
    while idx < args.epochs:
        ...
        if idx == args.epochs:
            ...
            plt.plot(range(args.epochs + 1), cache_efficiency_list, color='red', linewidth=1.5, linestyle='-',label='MCAF+RL')
            plt.plot(range(args.epochs + 1), cache_efficiency_without_list, color='blue', linewidth=1.5, linestyle='-',label='MCAF')
            plt.xlabel('Communication round')
            plt.ylabel('Cache hit radio')
            plt.legend()
            plt.show()
