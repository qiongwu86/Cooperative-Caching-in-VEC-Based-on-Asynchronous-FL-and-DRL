...
def vehicle_p_v(veh_dis,epoch_time):
    mu, sigma = 55,2.5
    lower, upper = mu - 2 * sigma, mu + 2 * sigma  # 截断在[μ-2σ, μ+2σ]
    x = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
    veh_speed = x.rvs(args.clients_num)
    ...
    for i in range(args.clients_num):
        ...
        if 0<veh_dis[i]<1000:
            all_pos_weight.append(veh_dis[i]/1000)
        if 1000<veh_dis[i]<2000:
            all_pos_weight.append((veh_dis[i]-1000)/1000)
        if 2000<veh_dis[i]<3000:
            all_pos_weight.append((veh_dis[i]-2000)/1000)
